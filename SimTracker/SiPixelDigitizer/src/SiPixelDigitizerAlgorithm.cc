...
//   }
  ...
  if(thePixelLuminosity==-1) {  // No inefficiency, all 100% efficient
	...
	thePixelChipEfficiency[i] = 1.-0.001; // chips = 99.9%
      ...
      //thePixelEfficiency[0] = 0.9;
      thePixelColEfficiency[0] = 0.9;
    }
    ...
    for (int i=0; i<6;i++) {
      thePixelEfficiency[i]    = 0.0;
      thePixelColEfficiency[i] = 0.0;
    }
  } else if (thePixelLuminosity<-1) {
    
    pixelInefficiency=true;
    for (int i=0; i<6;i++) {
      if(i<3) {  // For the barrel
	thePixelEfficiency[i]     = 1.;
	thePixelColEfficiency[i]  = 1.;
	thePixelChipEfficiency[i] = 1.;
      } else { // For the endcaps
	thePixelEfficiency[i]     = 1.;
	thePixelColEfficiency[i]  = 1.;
	thePixelChipEfficiency[i] = 1.;
      ...
      if (conf_.exists("thePixelColEfficiency_BPix1")) thePixelColEfficiency[0] = conf_.getParameter<double>("thePixelColEfficiency_BPix1");
      if (conf_.exists("thePixelColEfficiency_BPix2")) thePixelColEfficiency[1] = conf_.getParameter<double>("thePixelColEfficiency_BPix2");
      if (conf_.exists("thePixelColEfficiency_BPix3")) thePixelColEfficiency[2] = conf_.getParameter<double>("thePixelColEfficiency_BPix3");

      if (conf_.exists("thePixelEfficiency_BPix1")) thePixelEfficiency[0] = conf_.getParameter<double>("thePixelEfficiency_BPix1");
      if (conf_.exists("thePixelEfficiency_BPix2")) thePixelEfficiency[1] = conf_.getParameter<double>("thePixelEfficiency_BPix2");
      if (conf_.exists("thePixelEfficiency_BPix3")) thePixelEfficiency[2] = conf_.getParameter<double>("thePixelEfficiency_BPix3");

      if (conf_.exists("theBPix1LadderEfficiency")) theBPix1LadderEfficiency = conf_.getParameter<std::vector<double> >("theBPix1LadderEfficiency");
      if (conf_.exists("theBPix1ModuleEfficiency")) theBPix1ModuleEfficiency = conf_.getParameter<std::vector<double> >("theBPix1ModuleEfficiency");
      if (conf_.exists("theBPix1PUEfficiency")) theBPix1PUEfficiency = conf_.getParameter<std::vector<double> >("theBPix1PUEfficiency");

      std::cout<<"Custom dcol efficiency set as follows\n";
      std::cout<<"thePixelColEfficiency_BPix1 = "<<thePixelColEfficiency[0]<<std::endl;
      std::cout<<"thePixelColEfficiency_BPix2 = "<<thePixelColEfficiency[1]<<std::endl;
      std::cout<<"thePixelColEfficiency_BPix3 = "<<thePixelColEfficiency[2]<<std::endl;
      std::cout<<"thePixelEfficiency_BPix1 = "<<thePixelEfficiency[0]<<std::endl;
      std::cout<<"thePixelEfficiency_BPix2 = "<<thePixelEfficiency[1]<<std::endl;
      std::cout<<"thePixelEfficiency_BPix3 = "<<thePixelEfficiency[2]<<std::endl;

      std::cout<<"theBPix1LadderEfficiency:"<<std::endl;
      for (size_t i=0; i<theBPix1LadderEfficiency.size(); i++) {
	std::cout<<"\tLadder "<<i<<": "<< theBPix1LadderEfficiency[i]<<std::endl;
      }
      std::cout<<"theBPix1ModuleEfficiency:"<<std::endl;
      for (size_t i=0; i<theBPix1ModuleEfficiency.size(); i++) {
	std::cout<<"\tModule "<<i<<": "<< theBPix1ModuleEfficiency[i]<<std::endl;
      }
      std::cout<<"theBPix1PUEfficiency:"<<std::endl;
      for (size_t i=0; i<theBPix1PUEfficiency.size(); i++) {
	std::cout<<"\tPU "<<i<<": "<< theBPix1PUEfficiency[i]<<std::endl;
      }
    }
...
SiPixelDigitizerAlgorithm::run(
  ...
  if (puInfo.isValid() && theBPix1ModuleEfficiency.size()>0) {
    ...
    if (pu0!=puInfo->end()) {
      ...
      for (size_t i=0; i<theBPix1PUEfficiency.size(); i++) {
	//std::cout<<"PU scaling "<<_pu_scale<<std::endl;
	_pu_scale+=instlumi_pow*theBPix1PUEfficiency[i];
...
void SiPixelDigitizerAlgorithm::pixel_inefficiency() {
  ...
  if    (Subid==  PixelSubdetector::PixelBarrel){// barrel layers
    ...
    // VV:
    if ((layerIndex==1) && (theBPix1LadderEfficiency.size()==20)) {
      int ladder=PXBDetId(detID).ladder();
      //std::cout<<"BPix1 Ladder "<<ladder<<": "<<theBPix1LadderEfficiency[ladder-1]<<std::endl;
      columnEfficiency*=theBPix1LadderEfficiency[ladder-1];
    }
    if ((layerIndex==1) && (theBPix1ModuleEfficiency.size()==4)) {
      int module=PXBDetId(detID).module();
      if (module<=4) module=5-module;
      else module-=4;
      //std::cout<<"BPix1 Module "<<module<<": "<<theBPix1ModuleEfficiency[module-1]<<std::endl;
      columnEfficiency*=theBPix1ModuleEfficiency[module-1];
    }
    if ((layerIndex==1) && (_pu_scale>0.)) {
      columnEfficiency*=_pu_scale;
    }
...
//     ///////////////////////////Joco///////////////////////
//       float rnd  = flatDistribution_->fire();
...
void SiPixelDigitizerAlgorithm::module_killing_conf(void){
  ...
  for(signal_map_iterator i = _signal.begin();i != _signal.end(); i++) {
	...
	if (ladderC==quartLdr || ladderC==quartLdr+1 || ladderC==3*quartLdr || ladderC==3*quartLdr+1){// half modules
	  ...
	  else if((Module=="tbmB" || Module=="none") && ip.first >= 80){ //!!!bugfix miatt!!!
	    for (itDeadRocs = Dead_RocID.begin();itDeadRocs != Dead_RocID.end(); ++itDeadRocs) {
	      if (*itDeadRocs==-1)  {i->second.set(0.);}
	...
	if (ladderC==quartLdr || ladderC==quartLdr+1 || ladderC==3*quartLdr || ladderC==3*quartLdr+1){// half modules
	  ...
	  if((Module=="tbmB" || Module=="none") && ip.first < 80){//!!!bugfix miatt!!!
	    for (itDeadRocs = Dead_RocID.begin();itDeadRocs != Dead_RocID.end(); ++itDeadRocs) {
	      if (*itDeadRocs==-1)  {i->second.set(0.);}
	      else if (*itDeadRocs>7){ Dead_RocID2=*itDeadRocs%8;
		for(drId =0; drId < 8; drId++){
