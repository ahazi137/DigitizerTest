...
class SiPixelDigitizerAlgorithm  {
 public:
  ...
  edm::DetSet<PixelDigi>::collection_type  
    run(const std::vector<PSimHit> &input,PixelGeomDetUnit *pixdet,GlobalVector, edm::Handle<std::vector<PileupSummaryInfo> >&);
...
/*     } */
 ...
 private:
    ...
    // VV:
    std::vector<double> theBPix1LadderEfficiency;
    std::vector<double> theBPix1ModuleEfficiency;
    std::vector<double> theBPix1PUEfficiency;
